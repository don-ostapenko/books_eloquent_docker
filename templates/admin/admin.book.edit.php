<?php include __DIR__ . '/../parts/header.php'; ?>
    ...
    <div class="row mb-5">
        <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4 p-3 d-flex align-items-center" style="flex-direction: column">
            <form style="width: 100%" action="/admin/book/<?= $book->id ?>/edit" method="post">

                <div class="form-group">
                    <label for="email">Name *</label>
                    <input type="text" name="name" class="form-control <?php if ($errors['name']) { echo 'is-invalid'; } ?>"
                           id="name" aria-describedby="name"
                           placeholder="Enter name" value="<?= $book->name ?? $_POST['name'] ?>" required>
                    <?php if ($errors['name']): ?>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="price" class="form-control <?php if ($errors['price']) { echo 'is-invalid'; } ?>"
                           ...
                           placeholder="$ 0.00" value="<?= $book->price ?? $_POST['price'] ?>" required>
                    <?php if ($errors['price']): ?>
                        <div class="invalid-feedback">
                            <?= $errors['price'] ?>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="poster" class="form-control <?php if ($errors['poster']) { echo 'is-invalid'; } ?>"
                           ...
                           placeholder="Poster url" value="<?= $book->poster ?? $_POST['poster'] ?>">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="url" class="form-control <?php if ($errors['url']) { echo 'is-invalid'; } ?>"
                           ...
                           placeholder="Poster url" value="<?= $book->url ?? $_POST['url'] ?>">
                    <?php if ($errors['url']): ?>
                        <div class="invalid-feedback">
                            <?= $errors['url'] ?>
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="isbn" class="form-control <?php if ($errors['isbn']) { echo 'is-invalid'; } ?>"
                           ...
                           placeholder="Enter ISBN" value="<?= $book->isbn ?? $_POST['isbn'] ?>" minlength="10" maxlength="10" required>
<!--                    <small id="max" class="form-text text-muted" style="text-align: end"><span id="count">0</span> / 10</small>-->
                ...
                <div class="form-group">
                    ...
                    <div class="d-flex align-content-start">
                        <small id="tags" class="form-text text-muted d-block">
                            Current tags -
                            <?php if ($book->tags): ?>
                            <?php foreach ($book->tags as $tag): ?> <?= $tag->name ?> - <?php endforeach; ?>
                            <?php else: ?>
                            none
                            <?php endif; ?>
                        </small>
                ...
                <a class="btn btn-light" href="/admin" role="button">Cancel</a>
            </form>
        </div>
    </div>
